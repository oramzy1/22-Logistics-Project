...
import { CheckCircle2, Circle, CreditCard, Landmark, X } from 'lucide-react-native';
...
export default function PaymentScreen() {
  ...
  const [selectedType, setSelectedType] = useState<'Card' | 'Transfer' | null>(null);
...
const getPaystackUrl = () => {
  if (!authorizationUrl) return '';

  // Paystack accepts ?paymentChannel=card or ?paymentChannel=bank_transfer
  // to pre-select the tab on load
  const channelMap: Record<'Card' | 'Transfer', string> = {
    Card: 'card',
    Transfer: 'bank_transfer',
  };

  const channel = selectedType ? channelMap[selectedType] : null;
  if (!channel) return authorizationUrl;

  // Append as query param — Paystack reads this on their checkout page
  const separator = authorizationUrl.includes('?') ? '&' : '?';
  return `${authorizationUrl}${separator}paymentChannel=${channel}`;
};

const handleContinue = () => {
  if (!authorizationUrl || !selectedType) return;
  setShowWebView(true);
};
  ...
  if (verifying) {
    return (
      <SafeAreaView style={[styles.root, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ marginTop: 16, color: colors.muted }}>Verifying your payment...</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.content}>
        ...
        {/* Card Option */}
        <TouchableOpacity
          style={[styles.card, selectedType === 'Card' ? styles.selectedCard : styles.cardUnselected]}
          onPress={() => setSelectedType('Card')}
          activeOpacity={0.85}
        >
          <View style={styles.cardHeader}>
            <View style={styles.iconRow}>
              <CreditCard size={20} color={selectedType === 'Card' ? '#1D4ED8' : '#6B7280'} />
              <Text style={[styles.cardTitle, selectedType === 'Card' && styles.selectedCardTitle]}>
                Card
              </Text>
            </View>
            {selectedType === 'Card'
          ...
          {selectedType === 'Card' && (
            <TouchableOpacity style={styles.continueBtn} onPress={handleContinue}>
              <Text style={styles.continueBtnText}>Continue with Card →</Text>
            </TouchableOpacity>
          )}
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[styles.card, selectedType === 'Transfer' ? styles.selectedTransfer : styles.cardUnselected]}
          onPress={() => setSelectedType('Transfer')}
          activeOpacity={0.85}
        >
          <View style={styles.cardHeader}>
            <View style={styles.iconRow}>
              <Landmark size={20} color={selectedType === 'Transfer' ? '#974C16' : '#6B7280'} />
              <Text style={[styles.cardTitle, selectedType === 'Transfer' && styles.selectedTransferTitle]}>
                Bank Transfer
              </Text>
            </View>
          ...
          {selectedType === 'Transfer' && (
            <TouchableOpacity style={styles.continueBtn} onPress={handleContinue}>
              <Text style={styles.continueBtnText}>Continue with Transfer →</Text>
            </TouchableOpacity>
          )}
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={showWebView} animationType="slide" onRequestClose={() => setShowWebView(false)}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <WebView
            source={{ uri: getPaystackUrl() }}
            onNavigationStateChange={handleNavigationChange}
            startInLoadingState
            renderLoading={() => (
              <View style={styles.webViewLoader}>
                <ActivityIndicator size="large" color={colors.navy} />
                <Text style={{ marginTop: 12, color: colors.muted }}>Loading payment gateway...</Text>
              </View>
            )}
          />
        </SafeAreaView>
