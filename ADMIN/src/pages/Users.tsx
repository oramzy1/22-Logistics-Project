...
import { MoreHorizontal, Download, Building2, User, Loader2 } from "lucide-react";
...
import { useUsers, useSetUserStatus, useDeleteUser, useUpdateUserRole } from "@/hooks/useAdminData";
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        {([["BUSINESS", "Business"], ["INDIVIDUAL", "Individual"]] as const).map(([val, label]) => (
          <button
            key={val}
            onClick={() => { setTab(val); setParams(p => ({ ...p, page: '1' })); }}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-md text-sm border",
              tab === val ? "bg-accent/10 text-accent border-accent" : "bg-surface text-muted-foreground border-border"
            )}
          >
            {val === "BUSINESS" ? <Building2 className="h-4 w-4" /> : <User className="h-4 w-4" />}
            {label}
          </button>
        ))}
      ...
      <div className="bg-surface rounded-xl border border-border p-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3 text-xs">
          <div className="flex flex-wrap items-center gap-3">
            <span className="font-medium">{tab === 'BUSINESS' ? 'Business' : 'Individual'} ({total})</span>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-success" /> Active ({activeCount})</span>
            <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-muted-foreground" /> Inactive ({inactiveCount})</span>
           ...
           <thead>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="py-12 text-center text-muted-foreground text-sm">No users found</td></tr>
                ...
                <tr key={u.id} className="border-b border-border/60 last:border-0">
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <Avatar className="h-7 w-7">
                        <AvatarImage src={u.avatarUrl ?? u.businessProfile?.logoUrl} />
                  ...
                  {tab === "BUSINESS" && (
                    <td className="py-3 text-muted-foreground">{u.businessProfile?.companyName ?? '—'}</td>
                  )}
                  <td className="py-3 text-muted-foreground text-xs">{u.email}</td>
                  <td className="py-3">{u._count?.bookingsAsCustomer ?? 0}</td>
                  <td className="py-3"><StatusBadge status={u.isActive ? 'Active' : 'Inactive'} /></td>
                      ...
                      <DropdownMenuContent align="end">
                        {u.isActive ? (
                          <DropdownMenuItem
                            ...
                            onClick={() => setStatus.mutate({ id: u.id, isActive: false }, { onSuccess: () => toast.success('User deactivated') })}
                          >
                            Deactivate
                          </DropdownMenuItem>
                        ) : (
                          <DropdownMenuItem
                            onClick={() => setStatus.mutate({ id: u.id, isActive: true }, { onSuccess: () => toast.success('User reactivated') })}
                          >
                            Reactivate
                          </DropdownMenuItem>
                        )}
                        {u.role !== 'ADMIN' && (
                          <DropdownMenuItem
                            onClick={() => updateRole.mutate({ id: u.id, role: 'ADMIN' }, { onSuccess: () => toast.success('Upgraded to admin') })}
                          >
                            Upgrade to Admin
                          </DropdownMenuItem>
                        )}
                        <DropdownMenuItem
                          className="text-destructive"
                          onClick={() => deleteUser.mutate(u.id, { onSuccess: () => toast.success('User deleted') })}
                        >
                          Delete Account
                        </DropdownMenuItem>
                      </DropdownMenuContent>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 gap-3 text-sm">
          <div className="flex items-center gap-2">
            <span className="text-muted-foreground">Rows per page:</span>
             <select
              ...
              value={params.limit}
              onChange={(e) => setParams(p => ({ ...p, limit: e.target.value, page: '1' }))}
            >
              <option>10</option>
              <option>20</option>
              <option>50</option>
          ...
          <div className="flex items-center gap-1">
            <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setParams(p => ({ ...p, page: String(currentPage - 1) }))}>←</Button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
              <Button key={p} size="sm" variant={p === currentPage ? 'default' : 'outline'} onClick={() => setParams(prev => ({ ...prev, page: String(p) }))}>{p}</Button>
            ))}
            <Button variant="outline" size="sm" disabled={currentPage >= totalPages} onClick={() => setParams(p => ({ ...p, page: String(currentPage + 1) }))}>→</Button>
          </div>
        </div>
      </div>
    </div>
