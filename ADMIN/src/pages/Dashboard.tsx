...
const Dashboard = () => {
  ...
  const scheduledPct =
    pieTotal > 0 ? Math.round((breakdown.scheduled / pieTotal) * 100) : 0;
  const completedPct =
    pieTotal > 0 ? Math.round((breakdown.completed / pieTotal) * 100) : 0;
  const pieData = [
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-surface rounded-xl p-5 border border-border">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground capitalize">
                {rideType === "ALL" ? "All bookings" : rideType.toLowerCase()} ·{" "}
          ...
          <div className="flex justify-center gap-4 text-xs">
            ...
            <span className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-chart" /> Completed (
              {completedPct}%)
            </span>
          </div>
        </div>
        ...
        <div className="bg-surface rounded-xl p-5 border border-border">
          <div className="mb-3">
            ...
            <p className="text-xs text-muted-foreground">
              {PERIODS.find((p) => p.value === period)?.label} ·{" "}
              {rideType === "ALL" ? "All types" : rideType}
            </p>
          </div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
