...
const Drivers = () => {
  ...
  const { data: awaitingData } = useBookings({
  ...
  const handleExport = () =>
    exportCSV(
      "drivers",
      ["Name", "Email", "Phone", "Status", "Rating", "Trips", "License"],
      drivers.map((d: any) => [
        d.user?.name,
        d.user?.email,
        d.user?.phone,
  ...
  const canBeAssigned = (d: any) =>
    d.isOnline && d.onlineStatus !== "AWAY" && d.isAvailable;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <StatCard
          ...
          hint={`${availableCount} currently available`}
          hintTone="muted"
          icon={Truck}
        ...
        <StatCard
          ...
          value={String(stats?.pendingLicenses ?? 0)}
        ...
        />
        <StatCard
          label="Deactivated Drivers"
          value={String(deactivatedCount)}
          hint="Total deactivated"
      ...
      <div className="bg-surface rounded-xl border border-border p-5">
        <div className="flex flex-col lg:flex-row gap-3 mb-4">
          ...
          <select
            className="h-9 px-3 rounded-md border border-border bg-background text-sm"
            onChange={(e) =>
              setParams((p) => ({ ...p, isOnline: e.target.value, page: "1" }))
            }
          >
              ...
              {isLoading ? (
                ...
                drivers.map((d: any) => (
                  ...
                  >
                    ...
                    <td
                      className={`py-3 ${!d.user?.isActive ? "line-through text-muted-foreground" : ""}`}
                    >
                    ...
                    <td className="py-3">
                      <StatusBadge
                        status={
                          !d.user?.isActive ? "DEACTIVATED" : d.onlineStatus
                        }
                      />
                    </td>
                    <td className="py-3">
          ...
          {selected && (
            <div className="mt-4 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-sm">
                ...
                <Detail
                  label="Online Status"
                  value={<StatusBadge status={selected.onlineStatus} />}
                />
                <Detail
                  label="Rating"
                  value={
                ...
                <Detail
                  label="Total Trips"
                  value={String(selected.totalTrips)}
                />
                <Detail
                  label="Acceptance Rate"
                  value={`${selected.acceptanceRate ?? 0}%`}
                />
                <Detail
                  label="Vehicle Type"
              ...
              <div className="border-t border-border pt-4 flex gap-3">
                ...
                <Button
                  ...
                  className="flex-1 gap-2"
                  onClick={() =>
                    deleteUser.mutate(selected.userId, {
                      onSuccess: () => {
      ...
      <Dialog open={assignOpen} onOpenChange={setAssignOpen}>
        <DialogContent>
          ...
          <div className="py-2 space-y-3">
            <p className="text-sm text-muted-foreground">
              Select a booking awaiting a driver:
            </p>
                  ...
                  >
                    ...
                    <div className="text-sm">
                      <p className="font-medium">
                        {b.trackingId ?? b.id.slice(0, 8)} - {b.customer?.name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {b.packageType} · {b.pickupAddress}
                      </p>
                      <p className="text-xs text-muted-foreground">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!selectedBooking || assign.isPending}
            >
              {assign.isPending && (
