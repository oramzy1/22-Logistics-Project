...
const NAV_ITEMS = [
  ...
  },
  {
    id: "n3",
    label: "Drivers",
    description: "Driver management & license verification",
    path: "/drivers",
    icon: Truck,
    category: "Pages",
  },
  {
    id: "n4",
    label: "Users",
    description: "Individual & business user accounts",
    path: "/users",
    icon: Users,
    category: "Pages",
  },
  {
    id: "n5",
    label: "Payments",
    description: "Transactions, revenue & invoices",
    path: "/payment",
    icon: CreditCard,
    category: "Pages",
  },
  {
    id: "n6",
    label: "Settings",
    description: "Platform config & pricing",
    path: "/settings",
    icon: Settings,
    category: "Pages",
  },
...
export function CommandSearch({
  ...
  const bookingResults: Result[] = (bookingData?.bookings ?? []).map(
    (b: any) => ({
      id: `b-${b.id}`,
      label: b.trackingId ?? b.id.slice(0, 8),
  ...
  const select = useCallback(
    (r: Result) => {
      ...
      navigate(
        r.path,
        r.entityId ? { state: { highlightId: r.entityId } } : undefined,
      );
    },
    [navigate, onClose],
  );
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          <kbd className="hidden sm:inline-flex border border-border rounded px-1.5 py-0.5 text-[10px] font-mono text-muted-foreground">
            ESC
          </kbd>
        </div>
        ...
        <div className="max-h-[360px] overflow-y-auto py-2">
          ...
          {Object.entries(groups).map(([cat, results]) => (
            <div key={cat}>
              <p className="px-4 pt-2 pb-1 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">
                {cat}
              </p>
              {results.map((r) => {
        ...
        <div className="flex items-center gap-4 px-4 py-2 border-t border-border text-[10px] text-muted-foreground">
          {[
            ...
            <span key={k} className="flex items-center gap-1">
              <kbd className="border border-border rounded px-1 font-mono">
                {k}
              </kbd>
              {l}
            </span>
