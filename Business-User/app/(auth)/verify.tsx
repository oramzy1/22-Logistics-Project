...
import Animated, { useSharedValue, useAnimatedStyle, withSequence, withTiming, withRepeat, interpolateColor } from 'react-native-reanimated';
...
const handleVerify = async (enteredCode?: string) => {
  ...
  try {
    await AuthService.forgotPassword(email); // reuses same code-sending logic
  } catch (err) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.timerBadge}>
           <Text style={styles.timerText}>This code will expire in <Text style={styles.timerBold}>01:45</Text></Text>
        </View>
        {/* We wrap the entire row in a shake view */}
        <Animated.View style={[styles.codeContainer, animatedGroupStyle]}>
          {[0, 1, 2, 3, 4, 5].map((index) => (
            <Animated.View key={index} style={[styles.inputWrapper, animatedInputStyle]}>
              <TextInput 
...
<TouchableOpacity style={[styles.verifyBtn, isLoading && { opacity: 0.7 }]} onPress={() => handleVerify()} disabled={isLoading}>
...
const styles = StyleSheet.create({
  ...
  resendLink: { fontSize: 14, color: '#374151', textDecorationLine: 'underline', fontWeight: '500', marginBottom: 40 },
  verifyBtn: { backgroundColor: '#E4C77B', width: '100%', paddingVertical: 16, borderRadius: 8, alignItems: 'center' },
