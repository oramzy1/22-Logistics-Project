...
export default function ResetPasswordScreen() {
  ...
  const handleCodeContinue = async () => {
    ...
    try {
      ...
      setStep("newPassword"); // only advances on server confirmation
    } catch (err: any) {
      setError(err?.response?.data?.message || "Invalid or expired code.");
      setCode(""); // clear so they must re-enter
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      ...
      setError(
        err?.response?.data?.message ||
          "Reset failed. The code may have expired.",
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <>
              ...
              <View style={styles.inputContainer}>
                <TextInput
                  style={[
                    styles.input,
                    { letterSpacing: 6, fontSize: 18, fontWeight: "700" },
                  ]}
                  value={code}
                  onChangeText={(t) =>
