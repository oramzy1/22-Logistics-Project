...
import { View, StyleSheet, ScrollView, TouchableOpacity, Platform, RefreshControl, ActivityIndicator } from 'react-native';
...
import { NotificationService, AppNotification } from '@/api/notification.service';
 
// Map our DB notification types to icons
function getIcon(type: string) {
  switch (type) {
    case 'TRIP_STARTED':      return <Car size={20} color="#4B5563" />;
    case 'DRIVER_ASSIGNED':   return <UserPlus size={20} color="#4B5563" />;
    case 'PAYMENT_CONFIRMED': return <CreditCard size={20} color="#4B5563" />;
    case 'BOOKING_COMPLETED': return <CheckCheck size={20} color="#10B981" />;
    case 'TRIP_COMPLETED':    return <CheckCircle size={20} color="#10B981" />;
    case 'BOOKING_CANCELLED': return <AlertTriangle size={20} color="#F59E0B" />;
    case 'LICENSE_STATUS':    return <CheckCircle size={20} color="#3B82F6" />;
...
export default function NotificationsScreen() {
      ...
      <View style={styles.itemContent}>
        <View style={styles.itemHeader}>
          ...
          {!item.read
            ...
            : <Text style={styles.itemRightText}>{relativeTime(item.createdAt)}</Text>
          }
        </View>
        <Text style={
          item.type === 'DRIVER_ASSIGNED' ? styles.itemLinkDesc : styles.itemDesc
        }>{item.body}</Text>
        {!item.read && <Text style={styles.itemTime}>{relativeTime(item.createdAt)}</Text>}
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <ScrollView
        contentContainerStyle={[styles.content, displayed.length === 0 && { flex: 1, justifyContent: 'center' }]}
        showsVerticalScrollIndicator={false}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load(true)} colors={['#E4C77B']} />}
      >
        {displayed.length === 0 ? (
          <View style={{ alignItems: 'center', opacity: 0.4 }}>
            <Bell size={40} color="#9CA3AF" />
            <Text style={{ color: '#9CA3AF', marginTop: 12, fontSize: 14 }}>No notifications</Text>
          </View>
        ) : (
          <>
            <Text style={styles.summaryText}>
              You have <Text style={styles.summaryHighlight}>{unreadCount} unread</Text> notification{unreadCount !== 1 ? 's' : ''}
            </Text>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: '#0B1B2B', paddingHorizontal: 20,
    paddingTop: Platform.OS === 'android' ? 20 : 10, paddingBottom: 20,
  },
