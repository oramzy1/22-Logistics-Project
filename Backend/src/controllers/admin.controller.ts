...
export const getDashboardStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [
      totalBookings, bookingsYesterday,
      totalRevenue, revenueYesterday,
      activeDrivers, registeredUsers,
      ...
      prisma.booking.count({ where: { paymentStatus: 'PAID', createdAt: { lt: todayStart, gte: yesterdayStart } } }),
      prisma.booking.aggregate({ where: { paymentStatus: 'PAID' }, _sum: { totalAmount: true } }),
      prisma.booking.aggregate({ where: { paymentStatus: 'PAID', createdAt: { lt: todayStart, gte: yesterdayStart } }, _sum: { totalAmount: true } }),
...
export const getAllUsers = async (req: AuthRequest, res: Response) => {
  try {
    const { role, search, page = '1', limit = '20', isActive } = req.query as Record<string, string>;
    ...
    const [users, total] = await Promise.all([
      prisma.user.findMany({
        ...
        select: {
          ...
          driverProfile: { select: { licenseStatus: true, isOnline: true, rating: true } },
  ...
  try {
    const user = await prisma.user.findFirst({
      ...
      include: {
        ...
        _count: { select: { bookingsAsCustomer: true, bookingsAsDriver: true } },
      },
    });
...
export const updateUserRole = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await audit(req.user!.id, 'UPDATE_ROLE', 'USER', req.params.id, { newRole: role });
...
export const setUserActiveStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await audit(req.user!.id, isActive ? 'REACTIVATE_USER' : 'DEACTIVATE_USER', 'USER', req.params.id);
    getIO().to(`user:${req.params.id}`).emit('account:status_changed', { isActive });
...
export const adminDeleteUser = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id },
      include: { _count: { select: { bookingsAsCustomer: true, bookingsAsDriver: true } } },
    });
    if (!user) return res.status(404).json({ message: 'User not found' });

    const hasHistory = user._count.bookingsAsCustomer > 0 || user._count.bookingsAsDriver > 0;
    ...
    await audit(req.user!.id, hasHistory ? 'SOFT_DELETE_USER' : 'HARD_DELETE_USER', 'USER', id);
    res.json({ message: 'User deleted', method: hasHistory ? 'anonymized' : 'hard_deleted' });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
  }
};
...
export const getAllBookings = async (req: AuthRequest, res: Response) => {
  try {
    const { status, rideType, search, page = '1', limit = '20', dateFrom, dateTo } = req.query as Record<string, string>;
    const skip = (parseInt(page) - 1) * parseInt(limit);
...
export const getAllDrivers = async (req: AuthRequest, res: Response) => {
  try {
    const { licenseStatus, isOnline, page = '1', limit = '20' } = req.query as Record<string, string>;
    const skip = (parseInt(page) - 1) * parseInt(limit);
...
export const updateSettings = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!Array.isArray(updates)) return res.status(400).json({ message: 'settings must be an array' });
    ...
    await audit(req.user!.id, 'UPDATE_SETTINGS', 'SETTINGS', undefined, { keys: updates.map(u => u.key) });
...
export const createPromo = async (req: AuthRequest, res: Response) => {
  try {
    const {
      ...
      minBookingAmount, maxDiscount, usageLimit,
      expiresAt, targetType, targetUserIds, targetRole,
    } = req.body;
    ...
    await audit(req.user!.id, 'CREATE_PROMO', 'SETTINGS', promo.id, { code: promo.code });
...
export const togglePromo = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await audit(req.user!.id, updated.isActive ? 'ENABLE_PROMO' : 'DISABLE_PROMO', 'SETTINGS', promo.id);
    res.json({ message: `Promo ${updated.isActive ? 'enabled' : 'disabled'}`, promo: updated });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
  }
};
...
export const validatePromoCode = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!promo || !promo.isActive) return res.status(404).json({ message: 'Invalid or inactive promo code' });
    if (promo.expiresAt && new Date() > promo.expiresAt) return res.status(400).json({ message: 'Promo code has expired' });
    ...
    if (promo.minBookingAmount && bookingAmount < promo.minBookingAmount) {
      return res.status(400).json({ message: `Minimum booking amount is ₦${promo.minBookingAmount.toLocaleString()}` });
    }
    ...
    if (promo.targetType === 'INDIVIDUAL' && user?.role !== 'INDIVIDUAL') {
      return res.status(403).json({ message: 'This promo is for individual customers only' });
    }
    if (promo.targetType === 'BUSINESS' && user?.role !== 'BUSINESS') {
      return res.status(403).json({ message: 'This promo is for business customers only' });
    }

    // Check if user already used this promo
    const alreadyUsed = await prisma.promoUsage.findFirst({ where: { promoId: promo.id, userId } });
    if (alreadyUsed) return res.status(400).json({ message: 'You have already used this promo code' });
    ...
    res.json({ valid: true, discountAmount, finalAmount, promo: { code: promo.code, description: promo.description } });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
  }
};
...
export const recordPromoUsage = async (promoCode: string, userId: string, bookingId: string, discountAmount: number) => {
  const promo = await prisma.promoCode.findUnique({ where: { code: promoCode.toUpperCase() } });
  if (!promo) return;
  await prisma.promoUsage.create({ data: { promoId: promo.id, userId, bookingId, discountAmount } });
