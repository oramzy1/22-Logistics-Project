...
const MILESTONE_RULES = [
  { bookingCount: 1,  code: 'WELCOME10',  discountType: 'PERCENTAGE', discountValue: 10, description: '10% off your second ride!' },
  { bookingCount: 5,  code: 'LOYAL15',    discountType: 'PERCENTAGE', discountValue: 15, description: '15% off — thanks for 5 trips!' },
  { bookingCount: 10, code: 'VIP20',      discountType: 'PERCENTAGE', discountValue: 20, description: '20% off — you\'re a VIP!' },
  { bookingCount: 20, code: 'ELITE25',    discountType: 'PERCENTAGE', discountValue: 25, description: '25% off — Elite member reward!' },
];
...
async function getMilestoneSettings() {
  const keys = [
    'policy_welcome_enabled', 'policy_welcome_discount', 'policy_welcome_days',
...
export async function checkAndGrantMilestonePromo(userId: string) {
  ...
  const discount = parseFloat(settings[`milestone_${completedCount}_discount`] ?? '10');
  const days     = parseInt(settings[`milestone_${completedCount}_days`]     ?? '30');
 
  const prefixMap: Record<number, string> = { 1: 'WELCOME10', 5: 'LOYAL15', 10: 'VIP20', 20: 'ELITE25' };
  ...
  await sendPromoEmail(user.email, user.name, code, description, discount, 'PERCENTAGE', expiresAt);
}
 



export async function grantNewUserPromo(userId: string, email: string, name: string) {
  ...
  await sendPromoEmail(email, name, code, 'Welcome! 10% off your first ride.', 10, 'PERCENTAGE', expiresAt);
}