// app/(driver-tabs)/notifications.tsx
...
// ── Driver-specific icon + color map ──────────────────────────
function getIconConfig(type: string): { icon: React.ReactNode; bg: string } {
  switch (type) {
    ...
    case "LICENSE_REJECTED":
      return {
        icon: <ShieldX size={18} color="#EF4444" />,
        bg: "#FEF2F2",
      };
    default:
      return {
...
// Derive license icon from body text since we store one type for both outcomes
function resolveType(item: AppNotification): string {
  if (
    item.type === "LICENSE_STATUS" &&
    item.body.toLowerCase().includes("reject")
  ) {
...
export default function DriverNotificationsScreen() {
  ...
  const renderItem = (item: AppNotification) => {
    ...
    const { icon, bg } = getIconConfig(resolvedType);
    const isLicenseRejected = resolvedType === "LICENSE_REJECTED";
        ...
        <View style={styles.itemContent}>
          <View style={styles.itemHeader}>
            <Text
              style={[
                styles.itemTitle,
                isLicenseRejected && { color: "#EF4444" },
              ]}
            >
              {item.title}
            </Text>
